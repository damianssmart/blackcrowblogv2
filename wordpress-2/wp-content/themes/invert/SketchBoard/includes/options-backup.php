...
class OptionsFramework_Backup {
	...
	function OptionsFramework_Backup () {
		...
		$this->token = 'invert-options-backup';
	...
	function register_sketch_admin_screen () {
		...
		$this->admin_page = add_theme_page(__('Import / Export Options', 'invert'),__('Invert Backup Options', 'invert'),'edit_theme_options', $this->token, array( &$this, 'sketch_admin_screen' ) );
...
?>
	...
	<div class="sketch-backup">
		...
		<h2><?php _e( 'Invert Import / Export Theme Options','invert' ); ?></h2>
		...
		<div class="sketch-bkrp">
			...
			<div class="import">
				...
				<h3><?php _e( 'Import Settings','invert' ); ?></h3>

				

				<p><?php _e( 'If you have settings in a backup file on your computer, the Import / Export system can import those into this site. To get started, upload your backup file to import from below.','invert' ); ?></p>
					...
					<form enctype="multipart/form-data" method="post" action="<?php echo admin_url( 'admin.php?page=' . $this->token ); ?>">
						...
						<label for="OptionsFramework-import-file"><?php printf( __( 'Upload File: (Maximum Size: %s)','invert' ), ini_get( 'post_max_size' ) ); ?></label>
						...
						<input type="submit" class="button" value="<?php _e( 'Upload File and Import','invert' ); ?>" />
			...
			<div class="export">
				...
				<h3><?php _e( 'Export Settings','invert' ); ?></h3>

				

				<p><?php _e( 'When you click the button below, the Import / Export system will create a text file for you to save to your computer.' ,'invert'); ?></p>
				...
				<form method="post" action="<?php echo admin_url( 'admin.php?page=' . $this->token ); ?>">
					...
					<input type="submit" class="button" value="<?php _e( 'Download Export File','invert' ); ?>" />
...
<?php
	...
	function admin_notices () {
		...
		if ( isset( $_GET['error'] ) && $_GET['error'] == 'true' ) {
			...
			echo '<div id="message" class="error"><p>' . __( 'There was a problem importing your settings. Please Try again.','invert') . '</p></div>';

		} else if ( isset( $_GET['error-update'] ) && $_GET['error-update'] == 'true' ) {  

			echo '<div id="message" class="error"><p>' . __( 'Please make some changes in order to save them.','invert' ) . '</p></div>';

		} else if ( isset( $_GET['error-export'] ) && $_GET['error-export'] == 'true' ) {  

			echo '<div id="message" class="error"><p>' . __( 'There was a problem exporting your settings. Please Try again.','invert' ) . '</p></div>';

		} else if ( isset( $_GET['invalid'] ) && $_GET['invalid'] == 'true' ) {  

			echo '<div id="message" class="error"><p>' . __( 'The import file you\'ve provided is invalid. Please try again.','invert') . '</p></div>';

		} else if ( isset( $_GET['imported'] ) && $_GET['imported'] == 'true' ) {  

			echo '<div id="message" class="updated"><p>' . sprintf( __( 'Settings successfully imported. | Return to %sTheme Options%s', 'invert' ), '<a href="' . admin_url( 'admin.php?page=options-framework' ) . '">', '</a>' ) . '</p></div>';
